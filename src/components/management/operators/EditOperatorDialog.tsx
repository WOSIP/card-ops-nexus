...
import { UserPen, Mail, ShieldCheck, Image as ImageIcon, Save, Phone } from "lucide-react";
...
export const EditOperatorDialog = ({ isOpen, onClose, onUpdate, operator }: EditOperatorDialogProps) => {
  ...
  const onSubmit = (data: OperatorFormValues) => {
    ...
    const updatedOperator: Operator = {
      ...
      role: data.role,
      status: data.status,
      avatarUrl: data.avatarUrl || operator.avatarUrl,
    };
      ...
      <DialogContent className="sm:max-w-[600px] bg-card/95 backdrop-blur-2xl border-border/40 p-0 overflow-hidden rounded-3xl shadow-2xl">
        <div className="relative h-40 bg-gradient-to-br from-primary/20 via-background to-background flex items-center justify-center overflow-hidden border-b border-border/20">
          ...
          <div className="relative z-10 text-center space-y-2 px-6">
            ...
            <DialogTitle className="text-2xl font-black tracking-tight text-foreground">Edit Operator Profile</DialogTitle>
            <DialogDescription className="text-muted-foreground font-medium text-sm">Update credentials and access levels for this operator.</DialogDescription>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="John Doe" className="h-11 bg-white/5 border-border/40 rounded-xl" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <div className="md:col-span-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs font-black uppercase tracking-widest text-muted-foreground">Avatar URL</FormLabel>
                      <FormControl>
                        <div className="relative">
